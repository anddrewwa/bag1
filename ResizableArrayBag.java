...
public final class ResizableArrayBag<T> implements BagInterface<T>
{
   ...
   /** Gets the difference between two resizableArray bags.
       @param otherBag  The other bag
       @return  An resizableArrayBag containing the difference */
   public ResizableArrayBag<T> difference(ResizableArrayBag<T> otherBag){
      ResizableArrayBag<T> result = new ResizableArrayBag<>();

      //Traverse the bag and checks the frequencies of an element
      for (int i = 0; i < numberOfEntries; i++){
         T current = bag[i];
         int frequencyOtherBag = otherBag.getFrequencyOf(current);

         //If frequency is 0 it means that it's not in otherBag, so you add it to the result
         if(frequencyOtherBag == 0){
            result.add(current);
         }

         //If's not 0, you still need to check if there's more than 1 of the element because you need to add that difference
         else{
            int goTo = getFrequencyOf(current) - frequencyOtherBag;
      ...
      return result; //returns the resizableArrayBag with difference
   } //end of difference
